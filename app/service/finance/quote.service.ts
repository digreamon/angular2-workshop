...
export abstract class QuoteService {
    abstract loadQuotes(quotes:string):any;
}
...
export class QuoteServiceImpl extends QuoteService {
    ...
    loadQuotes(quotes:string):Observable<Quote[]|Quote> {
        return this.http.get(this.getUrl(quotes))
        .map(response => response.json().query.results.quote)
        .map(quotes => {
            if(quotes.map){
                return quotes.map(
...
export class QuoteServiceImplMock extends QuoteService {
    ...
    loadQuotes(quotes:string):Quote[] {
        return this.quotes.map(i => new Quote(i.symbol, i.name, i.open, i.last));
    }
